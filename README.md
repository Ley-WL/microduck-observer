...
这是页面显示标定，不是硬件归零，不写 EEPROM、不发送运动或扭矩指令。仅在线、新鲜、无故障且未暂停时可标定；反馈过期或离线时模型保持最后角度。清除后恢复参考姿势，刷新页面、切换设备/数据源或后端重启后需重新标定。整体 IMU 姿态与关节转动独立叠加。
...
标定仅在当前页面会话生效，刷新、切换设备/数据坐标或后端重启会清除，需要重新点击。暂停显示不会清除标定。
...
后端设置 `MICRODUCK_SERVO_PORT` 为 URT-2 串口路径即可启用，默认 1 Mbps、5 Hz 轮询。`MICRODUCK_SERVO_IDS` 默认为 `11,12,13,14,21,22,23,24`。留空串口配置时不访问舵机。配置示例见部署说明。可在页面逐颗标定显示参考零点，并检查旋转方向。
