...
所有已接入的实时遥测（IMU、舵机、系统状态、日志）均通过 WebSocket 更新；HTTP 仅用于初始化元信息、历史查询及调试操作。舵机帧到达后立即应用，曲线仍独立限频。
...
这是页面显示标定，不是硬件归零，不写 EEPROM、不发送运动或扭矩指令。仅在线、新鲜、无故障且未暂停时可标定；反馈过期或离线时模型保持最后角度。清除后恢复参考姿势，零点与方向按设备地址及数据源保存到当前浏览器，刷新和后端重启后自动恢复；更换舵机、修改硬件偏移或多圈编码器重置后应重新标定。整体 IMU 姿态与关节转动独立叠加。
...
IMU 标定保存到当前浏览器，同一后端运行期间刷新自动恢复；按设备、数据源、坐标系及后端启动 ID 隔离，后端重启后需重新标定。暂停显示不会清除标定。
