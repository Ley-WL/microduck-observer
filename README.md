...
“姿态与 IMU”页已预留 15 个飞特 HD-1910 舵机的信息表，按左腿 20–24、头颈与嘴 30–34、右腿 10–14 分组。单屏表格展示关节角度、电压、温度、电流、原始负载、在线状态，以及在线数量、平均电压、最高温度和故障数量汇总；协议还预留编码器、扭矩、故障和样本年龄字段。配置串口后读取指定 ID 的编码器、电压、温度、原始电流、负载和状态；不发送运动、扭矩或校准指令。未配置 ID 显示“未接入”，配置后无应答显示“无应答”，超龄反馈不显示为实时数据。编码器未校准为关节角度，电流保留原始单位。15 个物理舵机与 14 个模型关节分别统计，嘴部 ID 34 单独保留。

鸭子关节均为模型参考姿势。视频、ToF、完整遥测录制回放和身体坐标校准尚未实现。服务不会因浏览器断开而停止采集。
...
后端设置 `MICRODUCK_SERVO_PORT` 为 URT-2 串口路径即可启用，默认 1 Mbps、5 Hz 轮询。`MICRODUCK_SERVO_IDS` 默认为 `11,12,13,14,21,22,23,24`。留空串口配置时不访问舵机。配置示例见部署说明。3D 关节仍为参考姿势，需要完成零位及方向标定后再接入。
