...
class ObservationBuilder:
    ...
    def build(self, snapshot, last_action):
        ...
        if cal['imu'].get('mountingQuaternion') is not None:
            self.C=rotation(unit_quat(cal['imu']['mountingQuaternion'])).T
        target=rotation(unit_quat(cal['imu'].get('targetQuaternion',[0,0,0,1])))
        ...
        gravity=self.C @ rotation(qrel).T @ self.C.T @ target.T @ np.array([0.,0.,-1.])
