...
class CalibrationProcessTests(unittest.TestCase):
    def test_same_owner_executes_job_and_returns_result(self):
        from pathlib import Path
        commands, results = queue.Queue(1), queue.Queue(1)
        commands.put(({'rows': [{'id': 10}]}, 'backup.json'))
        bus = object()
        poller = ServoPoller('fake', [10], queue.Queue(1), threading.Event(), commands=commands, results=results)
        with patch('pose_calibration.calibrate_hardware', return_value=[{'id':10,'ok':True}]) as calibrate:
            poller.execute_command(bus)
            calibrate.assert_called_once_with(bus, {'rows':[{'id':10}]}, Path('backup.json'))
        self.assertEqual(results.get_nowait(), (True,[{'id':10,'ok':True}]))

    def test_missing_bus_rejects_job_without_hardware_write(self):
        commands, results = queue.Queue(1), queue.Queue(1)
        commands.put(({'rows':[]}, 'backup.json'))
        poller = ServoPoller('fake', [10], queue.Queue(1), threading.Event(), commands=commands, results=results)
        with patch('pose_calibration.calibrate_hardware') as calibrate:
            poller.execute_command(None)
            calibrate.assert_not_called()
        self.assertFalse(results.get_nowait()[0])

    def test_spawned_worker_reports_unavailable_and_releases_future(self):
        from servos import ServoSource
        import time
        source=ServoSource(None,'__nonexistent_serial_port__',[10])
        source.thread.start()
        try:
            future=source.submit_calibration({'rows':[{'id':10}]},'unused-backup.json')
            self.assertFalse(future.cancel())
            deadline=time.monotonic()+8
            while not future.done() and time.monotonic()<deadline:
                source.drain_results();time.sleep(.02)
            with self.assertRaises(ValueError):future.result(timeout=1)
